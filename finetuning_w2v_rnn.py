...
kernel_initializer=glorot_uniform(seed = 123)))
    ...
    x = Dense(128,kernel_initializer=he_uniform(seed=123), activation='relu',)(merge)
    # x = Dropout(0.2)(x)
...
def aux_esim_model(embed_matrix, model_weight_path):
    base_model = get_model(embed_matrix)
    base_model.load_weights(model_weight_path)
    input_q, input_a = base_model.inputs
    input_f = Input((19,))
    hidden_esim = base_model.get_layer(index=15).output
...
gc.collect()
K.clear_session()
weight_path = '/home/kesci/work/zhifeng/zhifeng_rnn_weight_1_0.668621638244629.h5'
